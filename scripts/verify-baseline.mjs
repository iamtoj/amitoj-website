...
if (checkpoint || migrationCheckpoint) {
  ...
  const staticSemanticRevisions = new Set(migrationCheckpoint ? [
    '/',
    '/about',
    '/contact',
    '/photography',
  ...
  for (const [path, expectedDigest] of publicBaseline.routes) {
    ...
    if (migrationCheckpoint && (editorialRoutes.has(path) || staticSemanticRevisions.has(path))) continue;
  ...
  if (migrationCheckpoint) {
    ...
    const migratedEntries = historicalMigratedEntries
      ...
      .map((entry) => {
        const key = entry.route.startsWith('/essays/')
          ? `essay:${entry.slug}`
          : entry.route.startsWith('/blog/')
            ? `note:${entry.slug}`
            : `annotation:${entry.slug}`;
    ...
    for (const entry of migratedEntries) {
      ...
      const revision = u7Contract.bodyRevisions.find(({ route }) => route === entry.route);
      if (revision) {
        ...
        assert.deepEqual(revision.from.rendered, frozenEvidence, `${entry.route} U7 transition must bind U3 rendered evidence`);
        assert.deepEqual(renderedEditorialBodyFingerprint(html), revision.to.rendered, `${entry.route} differs from reviewed U7 rendering`);
        assert.notDeepEqual(revision.from.rendered, revision.to.rendered, `${entry.route} rendered U7 revision must not be a no-op`);
      } else {
        assertRenderedEditorialBody(html, frozenRendered, entry.route);
      }
      ...
      const sourceDirectory = entry.route.startsWith('/essays/')
        ? 'essays'
        : entry.route.startsWith('/blog/')
          ? 'notes'
          : 'reading-annotations';
      const sourceText = await readFile(join(root, 'src/content', sourceDirectory, `${entry.slug}.md`), 'utf8');
      const frontmatter = sourceText.match(/^---\n([\s\S]*?)\n---/)?.[1] ?? '';
      const fields = Object.fromEntries(frontmatter.split('\n').map((line) => {
        const separator = line.indexOf(':');
        assert.ok(separator > 0, `${entry.route} has unsupported canonical frontmatter`);
        return [line.slice(0, separator), JSON.parse(line.slice(separator + 1).trim())];
      }));
      const kind = sourceDirectory === 'essays' ? 'essay' : sourceDirectory === 'notes' ? 'note' : 'annotation';
    ...
    const reviewedMetadataRevisions = [...networkContract.metadataRevisions, ...u7Contract.metadataRevisions]
...
console.log(
  migrationCheckpoint
    ? 'Migration checkpoint passed: immutable U1-U3 evidence remains valid; U7 publishes 47 reviewed editorial details and permanently redirects the retired Third Enlightenment Note.'
    : checkpoint

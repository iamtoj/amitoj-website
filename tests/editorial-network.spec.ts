...
const detailContracts = [
  ...
  },
  {
    ...
    form: 'Note',
    context: 'The night a model named a pattern I recognized, and why I still hesitate to call what happened a partnership.',
...
test('editorial detail headers separate navigation from the form label', async ({ page }) => {
  for (const contract of [
    { path: '/essays/the-right-direction', back: 'Back to Writing' },
    { path: '/blog/from-org-theory-to-ai', back: 'Back to Writing' },
    { path: '/library/breath', back: 'Back to Library' },
  ]) {
    await page.goto(contract.path);
    const backBox = await page.getByRole('link', { name: contract.back }).first().boundingBox();
    const labelBox = await page.locator('main [data-editorial-form]').first().boundingBox();
    expect(backBox).not.toBeNull();
    expect(labelBox).not.toBeNull();
    expect(labelBox!.y - (backBox!.y + backBox!.height)).toBeGreaterThanOrEqual(12);
  }
});

test('Note tags wrap between chips without breaking a chip at 320px', async ({ page }) => {
  await page.setViewportSize({ width: 320, height: 720 });
  await page.goto('/blog/from-org-theory-to-ai');

  const chips = page.locator('[data-note-tag]');
  ...
  for (const chip of await chips.all()) {
    ...
    expect(geometry.height).toBeLessThanOrEqual(geometry.lineHeight + 10);
  }
});

...
const u10 = JSON.parse(await readFile(join(root, 'tests/fixtures/u10-voice-revision-contract.json'), 'utf8'));
...
function reviewedRouteMetadata(route) {
  const revision = u10.renderedRouteRevisions.find(({ route: revisedRoute }) => revisedRoute === route);
  assert.ok(revision, `${route} needs reviewed U10 route evidence`);
  return { title: revision.to.title, description: revision.to.description };
}
  ...
  const contracts = [
    {
      ...
      ...reviewedRouteMetadata('/'),
    },
    {
      route: '/research',
      file: join(dist, 'research', 'index.html'),
      ...reviewedRouteMetadata('/research'),
    },
    {
      route: '/practices',
      file: join(dist, 'practices', 'index.html'),
      ...reviewedRouteMetadata('/practices'),
    },
    {
      route: '/coaching',
      file: join(dist, 'coaching', 'index.html'),
      ...reviewedRouteMetadata('/coaching'),
    },
    {
      route: '/yoga',
      file: join(dist, 'yoga', 'index.html'),
      ...reviewedRouteMetadata('/yoga'),
    },
    {
      route: '/photography',
      file: join(dist, 'photography', 'index.html'),
      ...reviewedRouteMetadata('/photography'),
    },
    {
      route: '/contact',
